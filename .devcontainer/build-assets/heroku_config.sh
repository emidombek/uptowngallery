...
echo
echo Get Ir Heroku API key by going to https://dashboard.heroku.com
echo Go to Account Settings and click on Reveal to view Ir Heroku API key
echo 

if [[ -z "${HEROKU_API_KEY}" ]]; then
   echo Paste Ir Heroku API key here or press Enter to quit:
   read apikey
   if [[ -z "${apikey}" ]]; then
      return 0
   fi
